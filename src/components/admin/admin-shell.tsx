  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-2 border-b border-border/60 bg-background/80 px-3 backdrop-blur-xl md:px-5">
      ...
      <Badge variant="outline" className="gap-1.5 border-primary/40 bg-primary/10 text-primary">
        <ShieldCheck className="h-3 w-3" /> {t("super_admin")}
      </Badge>
            ...
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">
                  {currentUser?.email ?? "admin@mdd.health"}
                </span>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem>{t("audit_log")}</DropdownMenuItem>
            <DropdownMenuItem>{t("security_settings")}</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
              onSelect={onLogout}
