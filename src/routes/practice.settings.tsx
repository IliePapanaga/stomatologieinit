  ...
  return (
    ...
    >
      <Card className="lg:col-span-2 border-border/70 shadow-sm">
        <CardHeader>
          <CardTitle className="text-base">{t("practice_details")}</CardTitle>
          <p className="text-xs text-muted-foreground">{t("practice_details_desc")}</p>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label={t("company_name")} defaultValue={mockPractice.companyName} />
            <Field label={t("status")} defaultValue="Active" disabled />
            <Field label={t("owner_first_name")} defaultValue={mockPractice.ownerFirstName} />
            <Field label={t("owner_last_name")} defaultValue={mockPractice.ownerLastName} />
            <Field label={t("email")} type="email" defaultValue={mockPractice.email} />
            <Field label={t("phone")} defaultValue={mockPractice.phone} />
          </div>
          <Separator />
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              onClick={() => toast.success(t("practice_details_saved"))}
            >
              {t("save_changes")}
            </Button>
          </div>
      ...
      <Card className="border-border/70 shadow-sm">
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">{t("account_desc")}</p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold">{t("growth_annual")}</p>
              <Badge className="bg-gradient-brand text-primary-foreground">{t("active")}</Badge>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              {t("renews_on", { date: "March 14, 2027" })}
            </p>
          </div>
          <Stat label={t("locations")} value={String(mockLocations.length)} />
          <Stat label={t("active_postings")} value="6" />
          <Stat label={t("team_seats")} value="4 / 10" />
        </CardContent>
      </Card>
    </motion.div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {mockLocations.length} {t("locations").toLowerCase()}
        </p>
        <Button
          className="bg-gradient-brand text-primary-foreground"
          onClick={() => toast("Open the location creator")}
        >
          <Plus className="h-4 w-4" /> {t("add_location")}
        </Button>
      </div>
            ...
            <Card className="overflow-hidden border-border/70 shadow-sm">
              ...
              <CardContent className="space-y-3 pt-4">
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  <Meta label={t("radius")} value={`${loc.radius} mi`} />
                  <Meta
                    label={t("contact")}
                    value={`${loc.contactFirstName} ${loc.contactLastName[0]}.`}
                  />
                  <Meta label={t("phone")} value={loc.phone.slice(-4)} prefix="•••• " />
                </div>
              </CardContent>
            </Card>
          </motion.div>
...
const initialPrefs: NotificationPref[] = [
  {
    ...
    description: "Someone applies to one of your postings",
    email: true,
    sms: false,
    push: true,
  },
  {
    ...
    description: "Professional checks in or out",
    email: false,
    sms: true,
    push: true,
  },
  {
    id: "noshow",
    label: "No-shows & late alerts",
    description: "Critical attendance events",
    email: true,
    sms: true,
    push: true,
  },
  {
    id: "sos",
    label: "SOS matched",
    description: "A professional accepts your emergency SOS",
    email: true,
    sms: true,
    push: true,
  },
  {
    ...
    description: "Successful payments and statements",
    email: true,
    sms: false,
    push: false,
  },
    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
      <Card className="border-border/70 shadow-sm">
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">{t("channels_desc")}</p>
        </CardHeader>
        <CardContent className="p-0">
          <div className="hidden grid-cols-[1fr_auto_auto_auto] gap-x-8 border-b border-border/60 px-6 py-3 text-[10px] font-medium uppercase tracking-wider text-muted-foreground sm:grid">
            ...
            <span className="flex items-center gap-1.5">
              <MessageSquare className="h-3 w-3" /> {t("sms")}
            </span>
            <span className="flex items-center gap-1.5">
              <Smartphone className="h-3 w-3" /> {t("push")}
            </span>
          </div>
          {prefs.map((p, i) => (
            <motion.div
            ...
            >
              <div className="min-w-0">
                <p className="text-sm font-medium">{t(`pref_${p.id}_label`, p.label)}</p>
                <p className="text-xs text-muted-foreground">{t(`pref_${p.id}_desc`, p.description)}</p>
              </div>
              <div className="flex gap-6 sm:contents">
                ...
                />
                <ChannelToggle label={t("sms")} checked={p.sms} onChange={() => toggle(p.id, "sms")} />
                <ChannelToggle
                  label={t("push")}
                  checked={p.push}
                  onChange={() => toggle(p.id, "push")}
                />
              </div>
            </motion.div>
          ))}
      ...
      <div className="mt-4 flex justify-end">
        <Button
          className="bg-gradient-brand text-primary-foreground"
          onClick={() => toast.success(t("notification_prefs_saved"))}
        >
          {t("save_preferences")}
        </Button>
      </div>
