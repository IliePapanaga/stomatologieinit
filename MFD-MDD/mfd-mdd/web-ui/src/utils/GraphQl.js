...
function parceFind(_levelA) {
    ...
    let propsA = _levelA.map(function (currentValue, index) {
        ...
        if (itemX instanceof QueryRequest) {
            ...
            if (Array.isArray(item)) {
                return new QueryRequest(propS).find(item)
            }
            if (item) {
                return `${propS} : ${item} `;
            } else {
...
function objectToString(obj) {
    ...
    for (let prop in obj) {
        ...
        let isBoolean = ('boolean' === typeof obj[prop]);
        if (obj[prop] || isBoolean) {
            let str = getGraphQLValue(obj[prop]);

            if (str || isBoolean) {
                sourceA.push(`${prop}:${str}`);
            }
...
function QueryRequest(_fnNameS, _aliasS_OR_Filter) {
...
QueryRequest.prototype = {
    ...
    toString: function () {
        ...
        return `${(this.aliasS) ? (this.aliasS + ":") : ""} ${this.fnNameS} ${(0 < this.headA.length) ? "(" + this.headA.join(",") + ")" : ""} ${bodyS}`;
    }
};
