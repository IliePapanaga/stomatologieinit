...
import static com.cl.mdd.server.core.event.impl.handler.payment.PaymentStatusEventHandler.PAYMENT_FAILED_NOTIFICATION;

@Consumer
public class FailedPaymentHandler implements EventHandler<PaymentStatusEvent> {

    private final PaymentStatusEventHandler delegate;

    @Autowired
    public FailedPaymentHandler(PaymentStatusEventHandler delegate) {
        this.delegate = delegate;
    }

    @Override
    @NotificationDefinition(value = PAYMENT_FAILED_NOTIFICATION,
            predefined = {
    ...
    public void onEvent(PaymentStatusEvent event, long sequence, boolean endOfBatch) {
        delegate.notifyPractice(event, PaymentAttempt.STATUS_FAILED);
    }
}
